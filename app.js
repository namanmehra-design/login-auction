...
function _reconstructSnapshotFromMatch(m, teams, mp){
 ...
 var xiCount=11;
 try{ if(roomState&&parseInt(roomState.activeSquadXI,10)) xiCount=parseInt(roomState.activeSquadXI,10); }catch(_e){}
...
async function _recalcLeaderboardCore(){
 ...
 Object.entries(matches).forEach(function(me){
  ...
  if(!matchSnaps || !Object.keys(matchSnaps).length){
   matchSnaps = _reconstructSnapshotFromMatch(m, teams, roomState.maxPlayers||16);
  }
  if(!matchSnaps){
...
window.saPopulateOsRooms=async function(){
  ...
  try{
    ...
    const auctionRooms=new Map(); // rid -> {ownerEmail, name}
...
window.saPopulateMultRooms=async function(){
  ...
  try{
    ...
    for(const [rid,info] of auctionRooms){
      ...
      const o=document.createElement('option');
      o.value='auction:'+rid;
      o.textContent=`${name}  ·  (${info.ownerEmail||'—'})  ·  auction  ·  ${rid.substring(0,6)}  ·  XI:${curMult}x`;
      sel.appendChild(o);
    }
    for(const [rid,info] of draftRooms){
      const nameSnap=await get(ref(db,`drafts/${rid}/roomName`));
      const name2=nameSnap.val()||info.name||'Untitled room';
...
const CBZ_PLAYER_IMG = {
  ...
  "mitchell santner":845505,"keshav maharaj":8993,"will jacks":848529,"corbin bosch":846101,"raj bawa":226295,
...
// window.proposeTrade reads off-screen DOM inside #app-shell (CD pushes
// it to left:-99999px, so it's unreachable). This takes selections
// directly. Returns a promise so the CD UI can react.
window.cdSubmitTradeA=function(partner, sending, receiving){
 if(!roomId||!myTeamName){ window.showAlert('Join a room first.','err'); return Promise.reject(new Error('no room')); }
 ...
 return push(ref(db,'auctions/'+roomId+'/trades'),trade).then(function(){
  window.showAlert('Trade proposed to '+partner+'! They need to accept.','ok');
  get(ref(db,'auctions/'+roomId)).then(function(snap){
   var d=snap.val(); if(d&&window.renderTrades) window.renderTrades(d);
  });
